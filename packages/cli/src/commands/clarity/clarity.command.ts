  ...
  />`;
    ...
    if (!layoutContent.includes("import Script from 'next/script'")) {
      layoutContent = layoutContent.replace(
        /import/,
        "import Script from 'next/script';\nimport { CLARITY_PROJECT_ID } from './clarity';\nimport",
      );
    } else if (!layoutContent.includes('import { CLARITY_PROJECT_ID }')) {
      layoutContent = layoutContent.replace(
        /import Script from 'next\/script'/,
        "import Script from 'next/script';\nimport { CLARITY_PROJECT_ID } from './clarity'",
      );
    ...
    if (layoutContent.includes('</head>')) {
      ...
      layoutContent = layoutContent.replace(
        '</head>',
        `${clarityScriptContent}\n  </head>`,
      );
    } else if (layoutContent.includes('<body')) {
      // body 태그가 있는 경우 body 태그 열기 후에 추가
      layoutContent = layoutContent.replace(
        /<body[^>]*>/,
        (match) => `${match}${clarityScriptContent}`,
      );
    } else {
      output.warn(
        'layout 파일에 head 또는 body 태그를 찾을 수 없습니다. 수동으로 Microsoft Clarity 스크립트를 추가해주세요.',
      );
    }
